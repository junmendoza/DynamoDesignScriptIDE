...
namespace Dynamo.Graph.Nodes
{
    ...
    {
        /// <summary>
        /// Get a list of variable names that are output ports
        /// </summary>
        /// <param name="stmtList"></param>
        /// <returns></returns>
        internal static List<string> GetOutputVariables(List<Statement> stmtList)
        {
            List<string> filteredList = new List<string>();
            foreach (Statement stmt in stmtList)
            {
                if (stmt.AstNode is BinaryExpressionNode)
                {
                    string lhsName = ((stmt.AstNode as BinaryExpressionNode).LeftNode as IdentifierNode).Name;
                    filteredList.Add(lhsName);
                }
            }
            return filteredList;
        }
