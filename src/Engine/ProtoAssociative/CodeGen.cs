...
#region SSA_INDEX_STMT_RIGHT
        ...
        private void EmitLanguageBlockNode(AssociativeNode node, ref ProtoCore.Type inferedType, ProtoCore.AssociativeGraph.GraphNode graphNode, ProtoCore.CompilerDefinitions.Associative.SubCompilePass subPass = ProtoCore.CompilerDefinitions.Associative.SubCompilePass.kNone)
        {
            if (IsParsingGlobal() || IsParsingGlobalFunctionBody() || IsParsingMemberFunctionBody() )
            {
                ...
                // Handle depndencies of this language block 
                bool areDependencyArgsSpecified = langblock.Dependencies.Count > 0;
                if (areDependencyArgsSpecified)
                {
                    // Use specified dependencies
                    List<AssociativeNode> dependencyNodes = new List<AssociativeNode>();
                    List<GraphNode> dependentList = new List<GraphNode>();
                    foreach (AssociativeNode depNode in langblock.Dependencies)
                    {
                        IdentifierNode identNode = depNode as IdentifierNode;
                        Validity.Assert(null != identNode);
                        
                        ProtoCore.DSASM.SymbolNode symbolnode = null;
                        bool isAccessible = false;
                        bool isAllocated = VerifyAllocation(identNode.Value, globalClassIndex, globalProcIndex, out symbolnode, out isAccessible);
                        if (isAllocated)
                        {
                            ProtoCore.AssociativeGraph.GraphNode dependentNode = new ProtoCore.AssociativeGraph.GraphNode();
                            dependentNode.PushSymbolReference(symbolnode);
                            dependentList.Add(dependentNode);
                        }
                    }
                    Validity.Assert(null != graphNode);
                    graphNode.dependentList = new List<GraphNode>();
                    graphNode.dependentList = dependentList;
                }
                // Use default dependencies inferred from the language block
