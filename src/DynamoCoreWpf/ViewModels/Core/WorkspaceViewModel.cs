...
namespace Dynamo.ViewModels
{
    ...
    public partial class WorkspaceViewModel : ViewModelBase
    {
        ...
        private string GenerateDSPath()
        {
            ...
            string workspaceFolder = workspace.FileName;

            // Get the filename only
            // Remove the trailing path
            int indexofLastToken = 0;
            indexofLastToken = workspaceFolder.LastIndexOf(@"\");
            workspaceFolder = workspaceFolder.Remove(0, indexofLastToken + 1);

            // Remove the extension
            indexofLastToken = workspaceFolder.LastIndexOf(@".");
            workspaceFolder = workspaceFolder.Remove(indexofLastToken);
            ...
            // Recursively delete the folder if it exisits
            if (System.IO.Directory.Exists(autogenDSWorkspacePath)) {
                System.IO.Directory.Delete(autogenDSWorkspacePath, true);
            }
