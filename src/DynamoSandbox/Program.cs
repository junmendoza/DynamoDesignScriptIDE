...
namespace DynamoSandbox
{
    ...
    internal class Program
    {
        ...
        private static void MakeStandaloneAndRun(string commandFilePath, out DynamoViewModel viewModel)
        {
            ...
            // Jun: This is meant to control any rendering in the bg.
            // If set to false, it is visual programming only, no bg required
            bool enableBgRendering = false;
            if (enableBgRendering)
            {
                DynamoModel model = Dynamo.Applications.StartupUtils.MakeModel(false);
                viewModel = DynamoViewModel.Start(
                     new DynamoViewModel.StartConfiguration()
                     {
                         CommandFilePath = commandFilePath,
                         DynamoModel = model,
                         Watch3DViewModel = HelixWatch3DViewModel.TryCreateHelixWatch3DViewModel(enableBgRendering, new Watch3DViewModelStartupParams(model), model.Logger)
                     },
                     enableBgRendering);
            }
            else
            {
                DynamoModel model = Dynamo.Applications.StartupUtils.MakeModel_LiteVersionPrototype();
                model.PreferenceSettings.IsBackgroundGridVisible = enableBgRendering;
                model.PreferenceSettings.IsBackgroundPreviewActive = enableBgRendering;
                viewModel = DynamoViewModel.Start(
                        new DynamoViewModel.StartConfiguration()
                        {
                            CommandFilePath = commandFilePath,
                            DynamoModel = model,
                            Watch3DViewModel = null
                        },
                        enableBgRendering);
            }
